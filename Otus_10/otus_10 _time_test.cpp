...
void MakeTest() {
    ...
    std::string str1 = boost::str(boost::format(" ол-во команд=[%1%], команд в блоке = [10]\n") % kCommandsNumber);
    file << str1;
    for (auto threadsNum = 1U; threadsNum <= 64; ++threadsNum) {
        std::string str = boost::str(boost::format("%1%. ол-во ниток=[%2%]\n") % threadsNum % threadsNum);
        auto start = std::chrono::high_resolution_clock::now();
        MakeTestIter(threadsNum, kCommandsNumber);
        auto stop = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start);
        str += boost::str(boost::format("  ¬рем€ работы=[%1%] миллисекунд.\n") % duration.count());
        file << str;
