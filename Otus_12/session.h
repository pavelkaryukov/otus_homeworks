...
private:
    ...
    void UseUniqueHandler(const std::string aStr) {
        if (m_UniqueHandler == 0) //Ћенива¤ инициализаци¤
            m_UniqueHandler = async::connect(3);//”никальные обработчики используютс¤ только дл¤ обработки динамических сообщений, стало быть размер балка не важен
